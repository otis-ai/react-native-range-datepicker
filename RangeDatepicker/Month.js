...
export default class Month extends React.Component {
		...
		do{
			...
			for (let i = 0; i < 7; i++) {
				...
				if (i == currDate.days() && currDate.month() == currMonth) {
					...
					if (startDate && untilDate && (startDate.format('YYYYMMDD') < currDate.format('YYYYMMDD')  && untilDate.format('YYYYMMDD') >= currDate.format('YYYYMMDD'))) {
						dayObject.type = 'between';
					}
	...
	render() {
		...
		return (
			<View>
				<Text style={[{ fontSize: 14, padding: 14 }, textStyleMonth]}>
